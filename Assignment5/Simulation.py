...
class Simulation:
    ...
    def _update_sensors(self) -> None:
        ...
        for r in self.robots:
            ...
            beacons = []
            for id_, landmark in enumerate(self.environment.landmarks):
                d = landmark - r.position
                distance = np.linalg.norm(d)
                if distance <= r.maximum_value_sensor:
                    angle = np.arctan2(d[1], d[0]) - r.angle
                    beacons.append(np.array([distance, angle, id_]))
            r.update_sensors(values, beacons)
