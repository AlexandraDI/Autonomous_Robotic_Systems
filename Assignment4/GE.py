...
class GESettings:
    ...
    Args:
        ...
        seed: random seed
        individual_fitness: if True the fitness functions computes the fitness
            on a single individual. Otherwise it compute the fitness of the
    ...
    keep_best: int = 5
    seed: int = 42
    size: Tuple[float] = (-10, 10)  # TODO remove
    individual_fitness: bool = True
...
class GE:
    ...
    def save(self, path: str, additional_data: Optional[List] = None) -> None:
        """
        Save the current state of the GE

        Args:
            path: destination
            additional_data: save other things together the GE
        """
    ...
    def _update_fitness(self) -> None:
        ...
        """
        if self._settings.individual_fitness:
            self._fitness = [
                self._settings.fitness_function(i, self._rng) for i in self._population
            ]
        else:
            self._fitness = list(
                self._settings.fitness_function(self._population, self._rng)
            )
...
def proportional_selection(
    ...
    if min_fitness < 0:
        probs = (np.array(fitness) - min_fitness) / (
            np.sum(fitness) - min_fitness * len(fitness)
        )
    else:
