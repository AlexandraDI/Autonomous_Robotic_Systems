...
class Visualization:
    ...
    def __init__(self, simulation: Simulation, minimum_update: float = 0.1) -> None:
        self.running = False
    ...
    def visualize_environment(self) -> None:
        ...
        Args:
            environment: the environment that the simulation got as parameter

        Returns:
            None.

        """
        for wall in environment.wall_points:
    ...
    def visualize_robot(self):
        ...
        for robot in self.simulation.robots:
            ...
            self._screen.blit(self._font.render(str(int(robot.v_left)), True, (0, 0, 0)), (robot.position[0], robot.position[1] - robot.radius/2))
            self._screen.blit(self._font.render(str(int(robot.v_right)), True, (0, 0, 0)), (robot.position[0], robot.position[1]))
...
if __name__ == "__main__":
    ...
    robot = Robot(
        position=np.array([350, 350, np.pi * 2]),
        radius=30,
        maximum_value_sensor=10000,
        acceleration=10000,
        sensors=Robot.make_sensors(8),
    )
    np.random.seed(37)
    ...
    # Visualization(simulation).start(True)
